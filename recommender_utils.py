...
# Load data
def load_data():
    movies = pd.read_csv("data/movies.dat", sep="::", engine="python", header=None, names=["movie_id", "title", "genres"], encoding='latin-1')
    ratings = pd.read_csv("data/ratings.dat", sep="::", engine="python", header=None, names=["user_id", "movie_id", "rating", "timestamp"], encoding='latin-1')
    users = pd.read_csv("data/users.dat", sep="::", engine="python", header=None, names=["user_id", "gender", "age", "occupation", "zip"], encoding='latin-1')
...
def clean_title(title):
    # Remove year and lower the case
...
def hybrid_recommend(user_id, movies, ratings, svd_model, cosine_sim, movieid_to_idx, top_n=10):
    if user_id not in ratings['user_id'].unique():
        return movies.iloc[np.argsort(-ratings.groupby('movie_id')['rating'].mean().values)[:top_n]][['movie_id', 'title']]
